        ...
        std::map< propagators::IntegratedStateType, orbit_determination::StateDerivativePartialsMap >( ) )
{
    ...
    {
        // CR3BP definition
        std::function< double( ) > gravitationalParameterPrimary = std::bind( &gravitation::GravityFieldModel::getGravitationalParameter,
                                                   bodyMap.at( secondaryBody )->getGravityFieldModel( ) );

        std::function< Eigen::Vector3d( ) > initialStatePrimary = std::bind( &simulation_setup::Body::getPosition, bodyMap.at( secondaryBody ) );

        std::function< double( ) > gravitationalParameterSecondary = std::bind( &gravitation::GravityFieldModel::getGravitationalParameter,
                                                     bodyMap.at( "Earth" )->getGravityFieldModel( ) );

        std::function< Eigen::Vector3d( ) > initialStateSecondary = std::bind( &simulation_setup::Body::getPosition, bodyMap.at( "Earth" ) );

        std::function< Eigen::Vector3d( ) > relativeSCState = std::bind( &simulation_setup::Body::getPosition, bodyMap.at( bodyWithProperty ) );

        variableFunction = std::bind( &circular_restricted_three_body_problem::positionCR3BP, relativeSCState, initialStatePrimary, initialStateSecondary,
